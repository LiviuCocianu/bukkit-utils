...
public class FireworkUtils {
    public static @NotNull Firework spawnRandomFirework(final Location loc, boolean instant) {
        ...
        final FireworkEffect effect = FireworkEffect.builder().flicker(random.nextBoolean())
                .withColor(getColor((int) Utils.randomRange("1-17")))
                .withFade(getColor((int) Utils.randomRange("1-17")))
                .with(FireworkEffect.Type.values()[random.nextInt(FireworkEffect.Type.values().length)])
                .trail(random.nextBoolean()).build();

        fireworkMeta.addEffect(effect);
        if(!instant) fireworkMeta.setPower(random.nextInt(2) + 1);

        firework.setFireworkMeta(fireworkMeta);

        if(instant) firework.detonate();
        return firework;
    }
